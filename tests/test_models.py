...
class TestProcess:
    ...
    def test_get_instance_graph__override(
        ...
        assert (
            f'\t"{task.name} {task.pk}" [style="filled, rounded, dashed"]' in graph.body
        )
        assert (
...
class TestTask:
    ...
    def test_start_next_tasks__multiple_next_tasks(self, db):
        ...
        tasks = task.start_next_tasks(
            next_nodes=[models.SplitJoinProcess.batman, models.SplitJoinProcess.robin]
        )
        assert len(tasks) == 2
    ...
    def test_start_next_tasks__custom_task_creation(self, db):
        ...
        tasks = task.start_next_tasks(next_nodes=[models.SplitJoinProcess.join])
        join1 = tasks[0]
