...
@contextmanager
def _lock(workflow_pk):
    """
    Non blocking lock to prevent raise conditions on workflows.
    ...
    the workflow state. This avoids tasks to be executed with the wrong
    workflow state and multiple tasks changing a value of the same workflow
    field.
    ...
    __lock = connection.lock(
        "joeflow_workflow_{}".format(workflow_pk),
